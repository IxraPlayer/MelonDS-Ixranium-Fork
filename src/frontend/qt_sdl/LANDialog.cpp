...
void LANStartHostDialog::done(int r)
{
    ...
    if (r == QDialog::Accepted)
    {
        ...
        {
            QMessageBox::warning(this, "MelonDS - Ixranium Fork", "Please enter a player name.");
            return;
        }
        ...
        {
            QMessageBox::warning(this, "MelonDS - Ixranium Fork", "Failed to start LAN game.");
            return;
        }
...
void LANStartClientDialog::onDirectConnect()
{
    if (ui->txtPlayerName->text().trimmed().isEmpty())
    {
        QMessageBox::warning(this, "MelonDS - Ixranium Fork", "Please enter a player name before connecting.");
        return;
    }
    ...
    if (!lan().StartClient(player.c_str(), hostname.c_str()))
    {
        ...
        QMessageBox::warning(this, "MelonDS - Ixranium Fork", msg);
        setEnabled(true);
        lan().StartDiscovery();
        return;
    }
...
void LANDialog::done(int r)
{
    ...
    if (showwarning)
    {
        if (QMessageBox::warning(this, "MelonDS - Ixranium Fork", "Really leave this LAN game?",
